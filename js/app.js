...
const AUTO_REMOVE_MERGE_COLS = new Set(
  ['C','D','F','H','I','J','K','L'].map(letter => letter.charCodeAt(0) - 65)
);
...
function detectColumn(headers, patterns, fallbackIndex=-1){
...
function detectStockColumns(rows){
  ...
  const sku=detectColumn(headers,[/sku/i,/รหัส/i,/code/i,/product/i,/item/i],0);
...
function updateCompareReady(){
  ...
  if(!data1 || !data2){
    $("status").textContent="● กรุณาเลือกไฟล์ ORDER และ Stock";
    if(info){
      if(data1){
  ...
  if(!stockSkuCol || !stockQtyCol || !orderSkuCol || !orderQtyCol){
    $("status").textContent="❌ ตรวจจับหัวคอลัมน์ SKU/จำนวนไม่ครบ";
  ...
  $("status").textContent="⚡ พบไฟล์ครบแล้ว — กำลังตรวจ ORDER กับ Stock อัตโนมัติ...";
...
function compareFiles(){
  ...
  try{
    ...
    compareResults.sort((a,b)=>String(a.sku).localeCompare(String(b.sku),undefined,{numeric:true,sensitivity:"base"}));
...
if(downloadShortBtn) downloadShortBtn.onclick=()=>downloadCompareSubset("short");
const downloadNoneBtn=document.getElementById("downloadNone");
if(downloadNoneBtn) downloadNoneBtn.onclick=()=>downloadCompareSubset("none");
...
function applyOrderTextAuto(){
  ...
  compareTextTimer=setTimeout(()=>{
    ...
    if(!rows.length){
      data2Text=null;
      refreshCombinedOrderData();
      $("status").textContent='❌ อ่านข้อความ ORDER ไม่พบข้อมูล';
      return;
    }
...
function clearAll(){
  ...
  if(data1 && stockSkuCol && stockQtyCol){
    ...
    $("status").textContent="● Stock ยังคงอยู่ — กรุณาเลือกไฟล์ ORDER";
  }else{
...
async function loadOrderFile(file) {
  try {
    ...
    const dbSaveBtn = document.getElementById('orderDbSave');
    if (dbSaveBtn) dbSaveBtn.disabled = false;
    updateOrderDbStatus();
    // บันทึกข้อมูลล่าสุดลงฐานข้อมูลในเครื่องอัตโนมัติ
    if (window.ORDER_DATABASE) {
      try {
        await window.ORDER_DATABASE.saveDataset(file.name, orderHeaders, orderRows);
        updateOrderDbStatus();
      } catch (dbErr) {
        console.warn('ORDER DB auto-save:', dbErr);
      }
    }
...
function buildSelectedTrackingGroups() {
  ...
  orderFilteredRows.forEach(row => {
    ...
    map.get(groupKey).values.add(tracking);
  });
...
function renderSelectedTrackingCopy() {
  ...
  const total = result.groups.reduce((sum, g) => sum + g.values.length, 0);
  ...
  note.textContent =
    `พบ ${total.toLocaleString()} เลข Tracking | ${groupCount.toLocaleString()} กลุ่ม SKU | เว้นบรรทัดเมื่อเปลี่ยนกลุ่ม`;

  preview.innerHTML = result.groups.map(group => {
    const values = group.values
      .map(v => `<div class="tracking-item">${escapeHtml(v)}</div>`)
      .join('');
    return `<div class="tracking-group">${escapeHtml(group.label || group.key)}</div>${values}`;
  }).join('<div style="height:10px"></div>');
}

async function copyAllDisplayedTracking() {
  const text = getSelectedTrackingClipboardText();
  if (!text) {
  ...
  const box = document.getElementById('orderTrackingPreview');
  if (!copied) {
    if (box) {
    ...
    alert('เบราว์เซอร์ไม่อนุญาตให้คัดลอกอัตโนมัติ\nระบบเลือกเลข Tracking ทั้งหมดให้แล้ว กด Ctrl+C เพื่อคัดลอก');
    return false;
  }
  ...
  if (typeof window.fxToast === 'function') {
    window.fxToast(`คัดลอก Tracking ทั้งหมด ${count.toLocaleString()} เลขแล้ว`, 'success');
  }
  if (typeof window.workHistoryLog === 'function') {
    window.workHistoryLog('filter', 'orderTool', `คัดลอก Tracking ทั้งหมด ${count.toLocaleString()} เลข`);
  }
  return true;
}
...
if (trackingPreviewBox) {
  trackingPreviewBox.title = 'คลิก 1 ครั้งเพื่อคัดลอกเลข Tracking ทั้งหมดที่แสดง';
  trackingPreviewBox.addEventListener('click', async (event) => {
    if (event.target.closest('.tracking-empty')) return;
    await copyAllDisplayedTracking();
  });
}
...
orderDownloadBtn.onclick = downloadOrderWord;
...
function buildOrderPivot() {
  ...
  orderFilteredRows.forEach(row => {
    ...
    if (existing) {
      ...
      map.set(key, { sku, qty: amount, groupName });
    }
  });

  orderPivotRows = [...map.values()].sort(compareOrderPivotRows);

  const skuHeader = orderHeaders[skuIndex] || "SKU Merchant";
  const qtyHeader = orderHeaders[qtyIndex] || "จำนวน";
  ...
  orderPivotRows.slice(0,1000).forEach(r => {
    html += `<tr><td>${escapeHtml(r.sku)}</td><td>${escapeHtml(formatPivotNumber(r.qty))}</td></tr>`;
  });
  html += `</tbody></table>`;
  if (orderPivotRows.length > 1000) {
    html += `<div style="padding:8px;text-align:center;color:#687d83;font-size:11px">แสดง 1,000 รายการแรก จาก ${orderPivotRows.length.toLocaleString()} รายการ</div>`;
  }
  box.innerHTML = html;
}
...
async function updateOrderDbStatus() {
  const status = document.getElementById('orderDbStatus');
  const info = document.getElementById('orderDbInfo');
  if (!status || !info || !window.ORDER_DATABASE) return;
  try {
    const items = await window.ORDER_DATABASE.listDatasets();
    const latest = items[0];
    status.textContent = latest
      ? `มีข้อมูล ${items.length} ชุด | ล่าสุด: ${latest.fileName}`
      : 'ยังไม่มีข้อมูลใน Database';
    info.textContent = latest
      ? `ข้อมูลล่าสุด ${Number(latest.rowCount || 0).toLocaleString()} รายการ | บันทึกเมื่อ ${new Date(latest.savedAt).toLocaleString('th-TH')}`
      : 'ข้อมูลจะถูกเก็บในฐานข้อมูลของเบราว์เซอร์เครื่องนี้ และเรียกใช้ซ้ำได้แม้รีเฟรชหน้า';
  } catch (e) {
    status.textContent = 'ไม่สามารถอ่าน Database ได้';
    console.warn(e);
  }
}

async function loadLatestOrderFromDatabase() {
  if (!window.ORDER_DATABASE) return;
  try {
    const latest = await window.ORDER_DATABASE.getLatest();
    if (!latest) {
      alert('ยังไม่มีข้อมูล ORDER ในฐานข้อมูล');
      return;
    }
    orderRows = latest.rows || [];
    orderHeaders = latest.headers || (orderRows[0] || []);
    orderWorkbook = null;
    document.getElementById('orderFileName').textContent = `🗄️ ${latest.fileName} (Database)`;
    document.getElementById('orderFileCount').textContent = 'DB';
    document.getElementById('orderTool').classList.add('has-file');
    document.getElementById('orderRowCount').textContent = Math.max(0, orderRows.length - 1).toLocaleString();
    buildOrderFilters();
    applyOrderFilters();
    orderDownloadBtn.disabled = false;
    orderPivotExcelDownloadBtn.disabled = false;
    const dbSaveBtn = document.getElementById('orderDbSave');
    if (dbSaveBtn) dbSaveBtn.disabled = false;
    document.getElementById('orderStatus').textContent = `🗄️ ดึงข้อมูลจาก Database แล้ว | ${Math.max(0, orderRows.length - 1).toLocaleString()} รายการ`;
    updateOrderDbStatus();
  } catch (e) {
    console.error(e);
    alert('ดึงข้อมูลจาก Database ไม่สำเร็จ\n\n' + e.message);
  }
}

async function saveCurrentOrderToDatabase() {
  if (!orderRows.length) {
    alert('กรุณาเลือกไฟล์ ORDER ก่อน');
    return;
  }
  try {
    await window.ORDER_DATABASE.saveDataset(orderFileName || 'ORDER', orderHeaders, orderRows);
    updateOrderDbStatus();
    document.getElementById('orderStatus').textContent = `💾 บันทึกเข้า Database แล้ว | ${Math.max(0, orderRows.length - 1).toLocaleString()} รายการ`;
  } catch (e) {
    alert('บันทึก Database ไม่สำเร็จ\n\n' + e.message);
  }
}

document.getElementById('orderDbLoad')?.addEventListener('click', loadLatestOrderFromDatabase);
document.getElementById('orderDbSave')?.addEventListener('click', saveCurrentOrderToDatabase);
document.getElementById('orderDbClear')?.addEventListener('click', async () => {
  if (!window.ORDER_DATABASE) return;
  if (!confirm('ต้องการล้างข้อมูล ORDER ทั้งหมดใน Database ของเครื่องนี้ใช่หรือไม่?')) return;
  try {
    await window.ORDER_DATABASE.clearAll();
    updateOrderDbStatus();
  } catch (e) { alert('ล้าง Database ไม่สำเร็จ\n\n' + e.message); }
});
window.addEventListener('load', updateOrderDbStatus);
...
(() => {
  ...
  if(top && !$('.fx-system')){
    ...
    box.innerHTML='<span class="fx-dot"></span><b>SYSTEM READY</b><span class="fx-clock"></span>';
    top.appendChild(box);
  }
  ...
  document.addEventListener('click',e=>{
    ...
    btn.style.position=btn.style.position==='static'?'relative':(btn.style.position||'relative');
  ...
  hot.className='fx-hotkey'; hot.textContent='Ctrl + K  •  เมนูคำสั่ง';
...
(() => {
  ...
  function lockButtonPosition(){btn.style.setProperty('position','fixed','important');btn.style.setProperty('top','auto','important');btn.style.setProperty('left','auto','important');btn.style.setProperty('right',window.innerWidth<=600?'12px':'20px','important');btn.style.setProperty('bottom',window.innerWidth<=600?'52px':'58px','important');btn.style.setProperty('transform','none','important');}
...
(function(){
  ...
  function scanSkuButtons(){
    $$('table').forEach(table=>{
      if(table.closest('#proDashPanel'))return;
...
/* Global click feedback: applies to non-button areas only */
(function initGlobalClickFeedback(){
  const ignoredTags = new Set(['BUTTON','A','SCRIPT','STYLE','HTML','BODY']);
  let pressedEl = null;
  let pressTimer = null;
  ...
  function getTarget(el){
    ...
    if (el.closest('button, a, [role="button"]')) return null;
    if (el.closest('input[type="file"]')) return null;
    return el.closest('.card, .panel, .section, .tab, .tab-btn, .drop-zone, .filter-item, .filter-option, .filter-column, .sku-cell, .table-row, td, th, input, textarea, select, label, .clickable, [data-clickable]') || el;
  }

  document.addEventListener('pointerdown', (e) => {
    const target = getTarget(e.target);
    if (!target) return;
    if (pressedEl && pressedEl !== target) pressedEl.classList.remove('click-press-feedback');
    pressedEl = target;
    pressedEl.classList.add('click-press-feedback');
  }, {passive:true});
  ...
  function clearPress(){
    ...
    const el = pressedEl;
    pressTimer = setTimeout(() => el.classList.remove('click-press-feedback'), 90);
    pressedEl = null;
  }
